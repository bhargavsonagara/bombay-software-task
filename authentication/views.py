...
def Register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            try:
                result = q.enqueue(send_an_email, 'Welcome mail', 'We are all really excited to welcome you to our team!', user.email)
            except:
                emai_send = threading.Thread(
                    target=send_an_email, args=['Welcome mail', 'We are all really excited to welcome you to our team!',user.email])
                emai_send.start()
...
def UserListing(request):
    paginator = Paginator(User.objects.all().order_by('-date_joined'), 5)
...
def Update_password_save(request):
    if(request.method == "POST"):
        ...
        if user is not None:
            ...
            try:
                result = q.enqueue(send_an_email, 'Password update', 'Your password has been changed.',user.email)
            except:
                emai_send = threading.Thread(
                target=send_an_email, args=['Password update', 'Your password has been changed.',user.email])
                emai_send.start()
            user.save()
...
def handler500(request):
    return render(request, '500-not-found.html', status=500)